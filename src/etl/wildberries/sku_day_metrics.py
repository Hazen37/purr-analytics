...
def load_sku_day_metrics(date_from: str, date_to: str) -> None:
    """
    Базовая витрина SKU/day для WB из факта заказов.
    impressions/views будут дополняться при подключении профильных WB reports.
    """
    print(f"[wb_metrics] rebuild range={date_from}..{date_to}")

    execute_query(
        """
        DELETE FROM wb_sku_day_metrics
